...
export function BottomNav({ currentView, onChangeView }: BottomNavProps) {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#0F1729]/95 backdrop-blur-xl border-t border-white/5 pb-1 pt-1 safe-area-bottom">
            <div className="max-w-md mx-auto flex justify-between items-center px-2 w-full">
                {navItems.map((item) => (
                    <button
                        ...
                        className={cn(
                            "flex flex-col items-center justify-center flex-1 h-12 space-y-0.5 transition-all duration-200 relative",
                            currentView === item.id
                                ? "text-[#E5FF53]"
                                : "text-gray-500 hover:text-gray-300"
                        )}
                    >
                        <item.icon className={cn("w-[18px] h-[18px]", currentView === item.id ? "stroke-[2.5px] drop-shadow-[0_0_8px_rgba(229,255,83,0.5)] transition-all" : "stroke-2")} />
